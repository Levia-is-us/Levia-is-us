...
def handle_chat_flow(messages_history: list, user_input: str, tool_caller) -> None:
    ...
    reply_info = get_initial_response(messages_history)
    
    # Handle different response types
    if reply_info["type"] == "direct_answer":
        handle_direct_answer(reply_info)
    elif reply_info["type"] == "intent_summary":
        handle_intent_summary(reply_info, messages_history, tool_caller)

def get_initial_response(messages_history: list) -> dict:
    """Get initial response from LLM"""
    prompt = [{"role": "system", "content": intents_system_prompt}] + messages_history
...
def handle_intent_summary(reply_info: dict, messages_history: list, tool_caller) -> None:
    ...
    process_existing_memories(high_score_memories, summary, execution_records_str, messages_history, tool_caller)
    # else:
